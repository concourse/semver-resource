...
* `debug`: *Optional.* Sets `GIT_TRACE`, `GIT_TRACE_PACKFILE`, and `GIT_CURL_VERBOSE` to get verbose output from calls to the git CLI
...
the version.
  ...
  * `identity_endpoint`, `username`, `user_id`, `password`, `passcode`, `domain_id`, `domain_name`, `tenant_id`, `tenant_name`, `allow_reauth`, `token_id`, `application_credential_id`, `application_credential_name`,`application_credential_secret`:
...
individual parameters, see [auth_options.go](https://github.com/gophercloud/gophercloud/blob/main/auth_options.go).
